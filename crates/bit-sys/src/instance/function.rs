...
impl Function {
    ...
    /// # Error
    ///
    /// * If fail to create a [Function], then [WasmEdgeError::Func(FuncError::Create)](bit_types::error::FuncError) is returned.
    ///
    /// # Example
    ///
    ...
    ///
    /// ```rust
    /// use bit_macro::sys_host_function;
    /// use bit_sys::{FuncType, Function, WasmValue, CallingFrame};
    /// use bit_types::{error::HostFuncError, ValType, WasmEdgeResult, NeverType};
    ///
    /// #[sys_host_function]
...
impl FuncType {
    ...
    ///
    /// ```rust
    /// use bit_sys::FuncType;
    /// use bit_types::ValType;
    ///
    /// let func_ty = FuncType::create(vec![ValType::I32;2], vec![ValType::I32]).expect("fail to create a FuncType");
    /// ```
...
impl From<bit_types::FuncType> for FuncType {
    fn from(ty: bit_types::FuncType) -> Self {
        ...
        FuncType::create(param_tys, ret_tys)
            .expect("[wasmedge-sys] Failed to convert bit_types::FuncType into bit_sys::FuncType.")
    }
}
impl From<FuncType> for bit_types::FuncType {
    fn from(ty: FuncType) -> Self {
        ...
        bit_types::FuncType::new(args, returns)
    }
}
...
mod tests {
    ...
    use bit_macro::sys_host_function;
    use bit_types::{NeverType, ValType};
    use std::{
        sync::{Arc, Mutex},
        thread,
    };
