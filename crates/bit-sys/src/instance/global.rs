...
impl Global {
    ...
    ///
    /// Notice that only the [Global] instance of [Mutability::Var](bit_types::Mutability::Var) type can be set a new value. Setting a new value for a [Global] of [Mutability::Const](bit_types::Mutability::Const) causes a failure.
    ///
    /// # Argument
    ///
    ...
    ///
    /// ```
    /// use bit_sys::{Global, GlobalType, WasmValue};
    /// use bit_types::{ValType, Mutability};
    ///
...
impl GlobalType {
    /// Create a new [GlobalType] to be associated with the given [ValType](bit_types::ValType) and [Mutability](bit_types::Mutability).
    ///
    /// # Arguments
    ///
    ...
    /// Returns the [Mutability](bit_types::Mutability) value of the [GlobalType].
...
impl From<bit_types::GlobalType> for GlobalType {
    fn from(ty: bit_types::GlobalType) -> Self {
        GlobalType::create(ty.value_ty(), ty.mutability()).expect(
            "[wasmedge-sys] Failed to convert bit_types::GlobalType into bit_sys::GlobalType.",
        )
    }
}
impl From<GlobalType> for bit_types::GlobalType {
    fn from(ty: GlobalType) -> Self {
        bit_types::GlobalType::new(ty.value_type(), ty.mutability())
    }
}
...
mod tests {
    use super::*;
    use bit_types::{Mutability, ValType};
    use std::{
        sync::{Arc, Mutex},
        thread,
    };
