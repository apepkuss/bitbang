...
use bit_types::error::{MemError, WasmEdgeError};
...
impl Memory {
    ...
    /// # Errors
    ///
    /// * If fail to create the memory instance, then [WasmEdgeError::Mem(MemError::Create)](bit_types::error::MemError) is returned.
    ///
    /// # Example
    ///
    /// ```
    /// use bit_sys::{MemType, Memory};
    ///
    ...
    ///
    /// ```
    /// use bit_sys::{Memory, MemType};
    /// use bit_types::error::{CoreError, CoreExecutionError, WasmEdgeError};
    ///
...
impl From<bit_types::MemoryType> for MemType {
    fn from(ty: bit_types::MemoryType) -> Self {
        MemType::create(ty.minimum(), ty.maximum(), ty.shared())
            .expect("[wasmedge-sys] Failed to convert bit_types::MemoryType into bit_sys::MemType.")
    }
}
impl From<MemType> for bit_types::MemoryType {
    fn from(ty: MemType) -> Self {
        bit_types::MemoryType::new(ty.min(), ty.max(), ty.shared())
            .expect("[wasmedge-sys] Failed to convert bit_sys::MemType into bit_types::MemoryType.")
    }
}
