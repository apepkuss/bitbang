...
impl Table {
    ...
    ///
    /// ```
    /// use bit_sys::{TableType, Table};
    /// use bit_types::RefType;
    ///
...
impl From<bit_types::TableType> for TableType {
    fn from(ty: bit_types::TableType) -> Self {
        TableType::create(ty.elem_ty(), ty.minimum(), ty.maximum()).expect(
            "[wasmedge] Failed to convert bit_types::TableType into wasmedge_sys::TableType.",
        )
    }
}
impl From<TableType> for bit_types::TableType {
    fn from(ty: TableType) -> Self {
        bit_types::TableType::new(ty.elem_ty(), ty.min(), ty.max())
    }
}
...
mod tests {
    ...
    use bit_types::{error::HostFuncError, NeverType, RefType, ValType};
    use std::{
        sync::{Arc, Mutex},
        thread,
    };
